...
export default async function ReceivedPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="dashboard-content sent-detail-content">
    ...
    <div className="sent-detail-heading"><div><div className="eyebrow muted">ROZMOWA · {events.length} WIADOMOŚCI</div><h1>{latest.subject.replace(/^(?:\s*re\s*:\s*)+/i, "")}</h1><p className="template-help">{latest.from.name} · {latest.from.address}</p><span className={`thread-status ${conversation.answered ? "answered" : "pending"}`}>{conversation.answered ? "Odpowiedziano" : "Do odpowiedzi"}</span></div>{(latest.replyTo.length > 0 || latest.from.address) && <Link className="primary-small-button" href={`/dashboard/wysylki?reply=${latest.key}`}>Odpowiedz</Link>}</div>
